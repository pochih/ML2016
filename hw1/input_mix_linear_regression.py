...
output = open('submission_mix_linux12_2', 'w')
thetaOut = open('theta_mix_linux12_2', 'w')
thetaData = open('theta_mix_linux14', 'r').read().split(',')
...
lamda = 51.51 	                # regularization rate
MAX_ITERATION = 50000
...
def gradient_descent(alpha, lamda, contTrainData, ep=0.0001, max_iter=10000):
    ...
    theta = thetaData
    # theta = np.random.uniform(-.01, .01, (18*9+1+18))
    ...
    while oldCost != cost:
        ...
        print("Iteration %d | Cost: %f | alpha: %.18f" % (iter, cost, alpha))

        # count gradient
    	gradient = np.zeros(18*9+1+18)
        for i in range(len(theta)):
        	gradient[i] += 2 * lamda * theta[i]
        gradient[0] = sum(loss)
        for i in range(len(contTrainData)):
            for j in range(18):
                gradient[j+163] += (loss[i] * (contTrainData[i][0][j*9+8] ** 2))
    		for k in range(18*9):
...
if __name__ == '__main__':
    ...
    for i in range(len(testData)):
        ...
        for k in range(18):
            sum += ((testData[i][k*9+8] ** 2) * theta[k+163])
